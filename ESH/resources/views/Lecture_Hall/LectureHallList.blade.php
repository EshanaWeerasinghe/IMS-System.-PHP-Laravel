@extends('layouts.master')
@section('content')

<div class="row">
    <div class="col-lg-6">
        <h3 class="page-header"><i class="fa fa-file-text-o"></i> Lecture Hall List</h3>
        <ol class="breadcrumb">
            <li><i class="fa fa-home"></i><a href="index.html">Home</a></li>
            <li><i class="icon_document_alt"></i>Lecture Hall</li>
            <li><i class="fa fa-file-text-o"></i>Lecture Hall List</li>
        </ol>
    </div>
</div>
<div class="panel col-lg-6">
<!--    <div class="panel-body">
        <form  method="GET" id="frm-search">
            <table>
                <tr>
                    <td>
                        <input type="search" name="search" class="form-control" value="{{request('search')}}" placeholder="Search By ID or Name">  
                    </td>
                </tr>
            </table>
                    
             </form>
        </div>
    -->
    <div class="panel-body">
           <table  class="table table-bordered table-condensed table-striped" id="hall-info">
           <col width ="50">
  
           
               <thead>
               <th style="text-align:center">No</th>
               <th style="text-align:center">Lecture Hall Name</th>
               <th style="text-align:center">Capacity</th>
               <th style="text-align:center;width: 100px">Action</th>
               
                 
               </thead>
               <tbody>
                   @foreach( $lecture_halls as $Key =>$stu)
                        <tr>
                            <td>{{ ++$Key }}</td>
                            <td>{{ $stu->hall_name }}</td>
                            <td style="text-align: center">{{ $stu->capacity}}</td>
                            <td>
                                <a class="btn btn-sm btn-success" href="{{route('editlec',$stu->hall_id)}}">Edit</a>
                                <a class="btn btn-sm btn-danger" onclick="getConfirmation();"  href="">Delete</a>                                  
                            </td>
                            
                            
                                                  
                        </tr>
                   @endforeach
               </tbody>
           </table>
        
        </div>
        <div class="footer">
        {{ $lecture_halls->render() }}
        </div>
   </div>

@endsection

@section('script')
<script type="text/javascript">
    $(document).ready(function(){
        $('#hall-info').DataTable({
            dom: 'Bfrtip',
            buttons: [
                'copyHtml5',
                'excelHtml5',
                'csvHtml5',
                'pdfHtml5'
            ]
            
        })
    })
</script>
@endsection